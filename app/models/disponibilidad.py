...
class Disponibilidad(Base):
    ...
    ultimaActualizacion = Column(
        "ultimaActualizacion", DateTime(timezone=True), nullable=False, server_default=func.now()
    )
    fuenteActualizacion = Column("fuenteActualizacion", String(50))

    habitacion = relationship("Habitacion", back_populates="disponibilidades")

    __table_args__ = (
        UniqueConstraint("habitacionId", "fecha", name="uq_disponibilidad_habitacion_fecha"),
